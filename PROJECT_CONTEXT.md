# 🎓 Project Context: Web-based Order Management System with Route Optimization

## 🎯 Суть та мета дипломної роботи

Розробка мультитенантної (SaaS) платформи для малого бізнесу (агрегатор/маркетплейс).
Система дозволяє постачальникам (SUPPLIER) створювати свої магазини та керувати товарами. Clientи (CLIENT) роблять
замовлення, а кур'єри (COURIER) отримують оптимізовані маршрути доставки на основі алгоритму графової оптимізації (
Дейкстра). Administrator (ADMIN) модерує магазини.
...
- **COURIER:** Отримує доступ до призначених йому замовлень та optimized route.
...
- Відношення: `1:N` (1 Supplier може мати багато Організацій).
